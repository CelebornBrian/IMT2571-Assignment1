...
class FunctionalTests extends \PHPUnit\Framework\TestCase
{
	...
	* @see generateTestCases */
	...
	protected function generateTestCases() 
	{
		...
		// The blank title and author test
		$cases[5] = array
					(
						'title' => '',
						'author' => '',
    ...
    public function testAdd()
    {
		...
		foreach ($this->generateTestCases() as $testCase)
		{
			...
			else
			{
				...
				$page = $this->session->getPage();
				$this->assertTrue($this->isExpectedPage($page, self::ERROR_PAGE_TITLE_IDX), 'testTitle: expecting error page');
			}
		}
	}
